...
import { ZScoreCalculator } from '@/features/statistics/z-score/ZScoreCalculator'
import { ZScoreSEOContent } from '@/features/statistics/z-score/ZScoreSEOContent'
...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: 'calculators.statistics.z-score',
  })
  const siteUrl =
    process.env.NEXT_PUBLIC_SITE_URL || 'https://www.worldcalculator.org'
  const calculatorPath = '/calculators/statistics/z-score'
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${siteUrl}/og/z-score-calculator.png`,
          width: 1200,
          height: 630,
          alt: t('title'),
        },
    ...
    twitter: {
      ...
      title: t('meta.title'),
      description: t('meta.description'),
      images: [`${siteUrl}/og/z-score-calculator.png`],
    },
  }
}
...
export default async function ZScoreCalculatorPage({
  ...
  const siteUrl =
    process.env.NEXT_PUBLIC_SITE_URL || 'https://www.worldcalculator.org'
  const calculatorUrl = `${siteUrl}/${locale}/calculators/statistics/z-score`
  ...
  const calculatorSchema = generateCalculatorSchema(
    {
      name: t('title'),
      description: t('meta.description'),
      url: `/${locale}/calculators/statistics/z-score`,
  ...
  const calculatorTranslations = {
    inputs: {
      mode: t('inputs.mode'),
      modeZScore: t('inputs.modeZScore'),
      modeZScoreDescription: t('inputs.modeZScoreDescription'),
      modeValue: t('inputs.modeValue'),
      modeValueDescription: t('inputs.modeValueDescription'),
      value: t('inputs.value'),
      valuePlaceholder: t('inputs.valuePlaceholder'),
      mean: t('inputs.mean'),
      meanPlaceholder: t('inputs.meanPlaceholder'),
      standardDeviation: t('inputs.standardDeviation'),
      standardDeviationPlaceholder: t('inputs.standardDeviationPlaceholder'),
      zScore: t('inputs.zScore'),
      zScorePlaceholder: t('inputs.zScorePlaceholder'),
    },
    results: {
      zScore: t('results.zScore'),
      value: t('results.value'),
      percentile: t('results.percentile'),
      standardDeviationsFromMean: t('results.standardDeviationsFromMean'),
      leftTailedPValue: t('results.leftTailedPValue'),
      rightTailedPValue: t('results.rightTailedPValue'),
      twoTailedPValue: t('results.twoTailedPValue'),
      confidenceLevel: t('results.confidenceLevel'),
    },
    interpretation: {
      belowMean: t('interpretation.belowMean'),
      atMean: t('interpretation.atMean'),
      aboveMean: t('interpretation.aboveMean'),
    },
    formula: t('formula'),
    pValuesTitle: t('pValuesTitle'),
    showPValues: t('showPValues'),
    hidePValues: t('hidePValues'),
    reset: t('reset'),
  ...
  const seoTranslations = {
    ...
    useCasesIntro: t('seo.useCasesIntro'),
    useCases: [
      t('seo.useCase1'),
      t('seo.useCase2'),
      t('seo.useCase3'),
  ...
  const calculatorConfig = getCalculatorBySlug('z-score')
  ...
  return (
    <>
      ...
      <CalculatorLayout
        ...
        seoContent={<ZScoreSEOContent translations={seoTranslations} />}
      >
        <ZScoreCalculator locale={locale} translations={calculatorTranslations} />
