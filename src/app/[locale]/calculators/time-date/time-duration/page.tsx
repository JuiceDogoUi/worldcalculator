...
import { TimeDurationCalculator } from '@/features/time-date/time-duration/TimeDurationCalculator'
import { TimeDurationSEOContent } from '@/features/time-date/time-duration/TimeDurationSEOContent'
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'calculators.timeDate.time-duration' })
  ...
  return {
    ...
    alternates: {
      canonical: `${siteUrl}/${locale}/calculators/time-date/time-duration`,
      languages: Object.fromEntries(locales.map((loc) => [loc, `${siteUrl}/${loc}/calculators/time-date/time-duration`])),
    },
    openGraph: { title: t('meta.title'), description: t('meta.description'), type: 'website', url: `${siteUrl}/${locale}/calculators/time-date/time-duration` },
    twitter: { card: 'summary_large_image', title: t('meta.title'), description: t('meta.description') },
  }
}

export default async function TimeDurationPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'calculators.timeDate.time-duration' })
  const tCategories = await getTranslations({ locale, namespace: 'categories' })
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.worldcalculator.org'
  const calculatorUrl = `${siteUrl}/${locale}/calculators/time-date/time-duration`

  const calculatorSchema = generateCalculatorSchema({ name: t('title'), description: t('meta.description'), url: `/${locale}/calculators/time-date/time-duration`, applicationCategory: 'UtilityApplication', offers: { price: '0', priceCurrency: 'USD' } }, { siteName: 'World Calculator', siteUrl, locale })

  const calculatorTranslations = {
    startTime: t('inputs.startTime'), endTime: t('inputs.endTime'), crossesMidnight: t('inputs.crossesMidnight'), reset: t('actions.reset'),
    results: { title: t('results.title'), duration: t('results.duration'), hours: t('results.hours'), minutes: t('results.minutes'), totalMinutes: t('results.totalMinutes'), totalSeconds: t('results.totalSeconds'), decimalHours: t('results.decimalHours') },
    validation: { startTimeRequired: t('validation.startTimeRequired'), startTimeInvalid: t('validation.startTimeInvalid'), endTimeRequired: t('validation.endTimeRequired'), endTimeInvalid: t('validation.endTimeInvalid') },
  }
  ...
  const sources = [{ title: 'Time calculation standards', url: 'https://www.nist.gov/pml/time-and-frequency-division' }]
  const calculatorConfig = getCalculatorBySlug('time-duration')
