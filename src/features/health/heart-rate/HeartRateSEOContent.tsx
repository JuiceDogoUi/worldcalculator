interface HeartRateSEOContentProps {
  translations: {
    whatIsTitle: string
    whatIsContent: string
    karvonenMethodTitle: string
    karvonenMethodContent: string
    karvonenMethodFormula: string
    zonesTitle: string
    zones: Array<{
      name: string
      intensity: string
      description: string
    }>
    howToMeasureTitle: string
    howToMeasureContent: string
    howToMeasureSteps: string[]
    howToUseTitle: string
    howToUseSteps: string[]
    tipsTitle: string
    tips: string[]
    faqTitle: string
    faqs: Array<{ question: string; answer: string }>
  }
}

export function HeartRateSEOContent({ translations: t }: HeartRateSEOContentProps) {
  return (
    <div className="space-y-10">
      {/* What is Target Heart Rate */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.whatIsTitle}</h2>
        <p className="text-muted-foreground leading-relaxed">{t.whatIsContent}</p>
      </section>

      {/* Karvonen Method */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.karvonenMethodTitle}</h2>
        <p className="text-muted-foreground mb-4">{t.karvonenMethodContent}</p>
        <div className="bg-muted p-4 rounded-lg space-y-2">
          <code className="text-sm font-mono block whitespace-pre-wrap">{t.karvonenMethodFormula}</code>
        </div>
      </section>

      {/* Heart Rate Zones */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.zonesTitle}</h2>
        <div className="grid gap-4">
          {t.zones.map((zone, index) => (
            <div key={index} className="p-4 border rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold">{zone.name}</span>
                <span className="text-sm bg-muted px-2 py-1 rounded">
                  {zone.intensity}
                </span>
              </div>
              <p className="text-muted-foreground text-sm">{zone.description}</p>
            </div>
          ))}
        </div>
      </section>

      {/* How to Measure Resting Heart Rate */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.howToMeasureTitle}</h2>
        <p className="text-muted-foreground mb-4">{t.howToMeasureContent}</p>
        <ol className="list-decimal list-inside space-y-2 text-muted-foreground">
          {t.howToMeasureSteps.map((step, index) => (
            <li key={index} className="leading-relaxed">{step}</li>
          ))}
        </ol>
      </section>

      {/* How to Use */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.howToUseTitle}</h2>
        <ol className="list-decimal list-inside space-y-2 text-muted-foreground">
          {t.howToUseSteps.map((step, index) => (
            <li key={index} className="leading-relaxed">{step}</li>
          ))}
        </ol>
      </section>

      {/* Tips */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.tipsTitle}</h2>
        <ul className="space-y-3">
          {t.tips.map((tip, index) => (
            <li key={index} className="flex items-start gap-3">
              <span className="flex-shrink-0 w-6 h-6 bg-primary text-primary-foreground rounded-full flex items-center justify-center text-sm font-medium">
                {index + 1}
              </span>
              <span className="text-muted-foreground">{tip}</span>
            </li>
          ))}
        </ul>
      </section>

      {/* FAQ */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.faqTitle}</h2>
        <div className="space-y-6">
          {t.faqs.map((faq, index) => (
            <div key={index} className="border-b pb-4 last:border-b-0">
              <h3 className="font-semibold mb-2 text-lg">{faq.question}</h3>
              <p className="text-muted-foreground">{faq.answer}</p>
            </div>
          ))}
        </div>
      </section>
    </div>
  )
}
