interface AreaSEOContentProps {
  translations: {
    ...
    commonConversionsTitle: string
    commonConversions: string[]
    realEstateTitle: string
    realEstateContent: string
    formulaTitle: string
...
export function AreaSEOContent({ translations: t }: AreaSEOContentProps) {
  return (
    <div className="space-y-10">
      ...
      {/* Unit Types */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.unitTypesTitle}</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          {t.unitTypes.map((type, index) => (
      ...
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.commonConversionsTitle}</h2>
        <div className="space-y-3">
          {t.commonConversions.map((conversion, index) => (
            <div key={index} className="p-4 border rounded-lg">
              <p className="text-muted-foreground font-mono text-sm">{conversion}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Real Estate Context */}
      <section>
        <h2 className="text-2xl font-bold mb-4">{t.realEstateTitle}</h2>
        <p className="text-muted-foreground leading-relaxed">{t.realEstateContent}</p>
      </section>
