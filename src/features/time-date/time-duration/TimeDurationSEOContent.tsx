...
interface TimeDurationSEOTranslations {
  whatIsTitle: string
...
interface TimeDurationSEOContentProps {
  translations: TimeDurationSEOTranslations
}

export function TimeDurationSEOContent({ translations: t }: TimeDurationSEOContentProps) {
  return (
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {t.features.map((feature, index) => (
            <Card key={index}><CardContent className="p-4"><p className="text-sm">{feature}</p></CardContent></Card>
          ))}
        </div>
      </section>
      <section>
